...
from helpers.helpers_train import compute_val_metrics_and_loss, _get_criterion, _get_optimizer
...
def run_training(
    ...
    """
    # loaders
    train_loader = prepare_loader_from_meta(train_meta, args.data_dir, args, augment=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_accuracy = train_one_epoch_with_criterion(model, train_loader, opt, device, criterion)
        ys, preds, probs, val_loss, val_acc = compute_val_metrics_and_loss(model, val_loader, device, criterion)
        val_macro = f1_score(ys, preds, average='macro') if len(ys) > 0 else 0.0
        rep = classification_report(ys, preds, target_names=["b","mb","h"], output_dict=True, zero_division=0)
        ...
        epoch_log = {
            ...
            "per_class_f1": {lab: float(rep.get(lab,{}).get("f1",0.0)) for lab in ["b","mb","h"]},
            "epoch_time_seconds": float(epoch_time),
            "eta_seconds": float(eta_seconds)
        }
        epoch_logs.append(epoch_log)

        logging.info(f"[{fold_tag}] epoch {epoch}/{args.epochs} train_loss={train_loss:.4f} train_acc={train_accuracy:.4f} val_loss={val_loss:.4f} val_macro_f1={val_macro:.4f} val_acc={val_acc:.4f} epoch_time={epoch_time:.2f}s")
        ...
        try:
            ...
            for lab in ["b","mb","h"]:
                tracker_obj.log_metric(f"f1_{lab}", float(epoch_log["per_class_f1"][lab]), step=step)
        except Exception:
            pass
        ...
        if val_acc > best_acc:
            ...
            torch.save({"model_state": model.state_dict(), "metadata": ckpt_meta}, best_path)
...
def train(meta, args_ns, device, artifacts_dir, tracker_obj, params, ds_hash, git_sha):
    ...
    if test_split and test_split > 0.0:
        remaining, test_meta = train_test_split(meta, test_size=test_split, stratify=stratify_arg, random_state=SEED)
        stratify_arg_rem = remaining['label'].map(LABEL_MAP).values if len(np.unique(remaining['label'].map(LABEL_MAP).values)) > 1 else None
        train_meta, val_meta = train_test_split(remaining, test_size=val_split / (1.0 - test_split), stratify=stratify_arg_rem, random_state=SEED)
        test_meta = test_meta.reset_index(drop=True)
    else:
        train_meta, val_meta = train_test_split(meta, test_size=val_split, stratify=stratify_arg, random_state=SEED)
        test_meta = None

    train_meta = train_meta.reset_index(drop=True)
    val_meta = val_meta.reset_index(drop=True)

    logging.info(f"Training (none mode) on {len(train_meta)} samples, validating on {len(val_meta)} samples" + (f", testing on {len(test_meta)} samples" if test_meta is not None else ""))

    fold_art = run_training(train_meta, val_meta, fold_tag="single", args=args_ns, device=device, artifacts_dir=artifacts_dir,
                                tracker_obj=tracker_obj, params=params, ds_hash=ds_hash, git_sha=git_sha)
    ...
    if test_meta is not None:
        ...
        model.load_state_dict(state["model_state"])
        criterion = _get_criterion(train_meta, device)
        test_loader = prepare_loader_from_meta(test_meta, args_ns.data_dir, args_ns, augment=False)
        ys, preds, probs, test_loss, test_acc = compute_val_metrics_and_loss(model, test_loader, device, criterion)
        rep = classification_report(ys, preds, target_names=["b","mb","h"], output_dict=True, zero_division=0)
        cm = confusion_matrix(ys, preds).tolist()
