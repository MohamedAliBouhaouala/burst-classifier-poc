...
def generate_pr_and_calibration(
    ...
    N, C = probs_arr.shape
    if len(classes) != C:
        warnings.warn(f"classes length {len(classes)} != probs shape {C}; adjusting classes to indices")
    ...
    try:
        ...
        if tracker:
            try: tracker.log_artifact(path_pr, name=os.path.basename(path_pr))
    ...
    try:
        ...
        if tracker:
            try: tracker.log_artifact(path_rel, name=os.path.basename(path_rel))
            except Exception: pass
    except Exception as e:
    ...
    try:
        ...
        if tracker:
            try: tracker.log_artifact(path_hist, name=os.path.basename(path_hist))
            except Exception: pass
    except Exception as e:
...
def generate_plots_from_summary(
    ...
    try:
        ...
        if tracker:
            try: tracker.log_artifact(p, name=os.path.basename(p))
            except Exception: pass
    except Exception as e:
    ...
    try:
        ...
        ax.bar(x, [0 if math.isnan(v) else v for v in values], tick_label=classes, color=colors)
        ax.set_ylim(0.0, 1.0)
    ...
    try:
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow(["accuracy", f"{accuracy:.6f}" if not math.isnan(accuracy) else ""])
            writer.writerow(["macro_f1", f"{macro_f1:.6f}" if not math.isnan(macro_f1) else ""])
            writer.writerow(["micro_f1", f"{micro_f1:.6f}" if not math.isnan(micro_f1) else ""])
            writer.writerow(["ece", f"{ece:.6f}" if not math.isnan(ece) else ""])
        saved["summary_csv"] = csv_path
        if tracker:
            try: tracker.log_artifact(csv_path, name=os.path.basename(csv_path))
            except Exception: pass
    except Exception as e:
