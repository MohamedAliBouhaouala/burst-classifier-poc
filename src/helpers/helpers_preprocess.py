...
def prepare_loader_from_meta(meta_df, data_dir, args, augment):
    durations = (meta_df['end_seconds'] - meta_df['start_seconds']).astype(float).clip(lower=1e-6)
    if durations.empty:
    ...
    print(f"Computed fixed_seconds for dataset (max): {computed_fixed_seconds:.3f}s (label max={max_dur:.3f}s, cli={args.fixed_seconds})")

    ds = SegmentDataset(meta_df, audio_root=data_dir, fixed_seconds=computed_fixed_seconds, n_mels=args.n_mels, augment=augment, seed=SEED)
